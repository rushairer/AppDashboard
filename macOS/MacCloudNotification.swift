...
final public class MacCloudNotification {
    
    private(set) lazy var publicSubscriptionIDKey: String = {
        return"\(self.container.containerIdentifier!.description).\(Self.cloudNotification).subscription.public"
    }()
    ...
    public init() {
        ...
        let subscription = CKQuerySubscription(recordType: Self.cloudNotification,
                                               predicate: predicate,
                                               subscriptionID: self.publicSubscriptionIDKey,
